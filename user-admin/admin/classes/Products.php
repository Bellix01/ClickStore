...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/home/user-admin/product_images/".$uniqueImageName)) {
					// $q = $this->con->query("SELECT * FROM products WHERE product_title = '$product_name' LIMIT 1");
					// if ($q->num_rows > 0) {
					// 	return ['status'=> 303, 'message'=> 'Le produit existe déjà'];
					// }else{
					$q = $this->con->query("INSERT INTO products (`product_cat`, `product_brand`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `product_keywords`)
					                        VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords')");

					if ($q) {
						return ['status'=> 202, 'message'=> 'Produit ajouté avec succès.. !'];
				...
				return ['status'=> 303, 'message'=> 'Grande image, taille maximale autorisée: 2Mo'];
			}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/home/user-admin/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Produit modifié avec succès.. !'];
					}else{
						return ['status'=> 303, 'message'=> 'Échec de l\'exécution de la requête'];
					}
			...
			return ['status'=> 303, 'message'=> 'Format d\'image invalide [Valid Formats : jpg, jpeg, png]'];
		}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			return ['status'=> 303, 'message'=> 'Identifiant de produit non valide'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'La catégorie existe déjà'];
		}else{
			$q = $this->con->query("INSERT INTO categories (cat_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nouvelle catégorie ajoutée avec succès'];
			}else{
				return ['status'=> 303, 'message'=> 'Échec de l\'exécution de la requête'];
			}
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Produit retiré de stock'];
			}else{
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'Identifiant de catégorie invalide'];
		}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'La marque existe déjà'];
		}else{
			$q = $this->con->query("INSERT INTO brands (brand_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Nouvelle marque ajoutée avec succès'];
			}else{
				return ['status'=> 303, 'message'=> 'Échec de l\'exécution de la requête'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Marque supprimée'];
			}else{
				return ['status'=> 202, 'message'=> 'Échec de l\'exécution de la requête'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Identifiant de marque invalide'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Marque mise à jour'];
			}else{
				return ['status'=> 202, 'message'=> 'Échec de l\'exécution de la requête'];
			}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			...
			echo json_encode(['status'=> 303, 'message'=> 'Identifiant de produit non valide']);
			exit();
		}
	}else{
