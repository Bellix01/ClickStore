	...
	public function loginAdmin($email, $password){
		...
		if ($q->num_rows > 0) {
			...
			if (password_verify($password, $row['password'])) {
				...
				return ['status'=> 202, 'message'=> 'Connexion réussie'];
			}else{
				return ['status'=> 303, 'message'=> 'Échec de la connexion'];
			}
		}else{
...
if (isset($_POST['admin_register'])) {
	...
	if (!empty($name) && !empty($email) && !empty($password) && !empty($cpassword)) {
		if ($password == $cpassword) {
			...
			echo json_encode(['status'=> 303, 'message'=> 'Les mot de passe sont différents, Réssayer']);
			exit();
		}
	}else{
